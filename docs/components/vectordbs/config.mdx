...
- `vector_store`: Specifies the vector database provider and its configuration
  - `provider`: The name of the vector database (e.g., "chroma", "pgvector", "qdrant", "milvus", "upstash_vector", "azure_ai_search", "vertex_ai_vector_search", "valkey", "lancedb")
...
| `uri` | Filesystem path or LanceDB URI |
...
| `table_name` | Override the underlying table name |
| `distance_metric` | Search distance metric (for LanceDB) |
| `storage_options` | Storage backend options passed to the provider |
